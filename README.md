...
| `--min-lines` | `5` | Skip chunks with fewer than N code lines; comments, blank lines, imports, and C preprocessor lines don't count |
...
Preprocessor lines — `#include`, conditional compilation, `#define` —
are configuration rather than code, so they are left out of similarity
(as PMD CPD does): two functions that differ only in their `#ifdef`
