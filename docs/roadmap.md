...
| 4 | Refactor patch emission | **Shipped (Go, Python, Java, JS/TS, Rust, Elixir, C)** | `--suggest <pair-id>`, `--suggest-all`, `id` + `suggested_patch` in JSON |
...
| C | **Shipped** | Starter helper with `//`-comment divergence block. The header is carried verbatim — storage class, attributes, return type, GNU-style line breaks, K&R parameter declarations, and `#ifdef` header variants — with every occurrence of the name renamed. The patch inserts the helper above A's function and its doc comment (C needs a declaration before use). Control-flow keyword set adds `goto`. Macro-generated definitions (`TEST(suite, name) { … }`) are rejected. Fixtures: simple, medium, advanced (`goto` cleanup), realworld-gnu, reject-controlflow; every patched fixture compiles with gcc. |
| Assembly | **Not planned** | `--suggest` rejects assembly pairs with a note to factor the shared instructions into an assembler macro; there is no function abstraction to extract into. Detection, partial clones, and dead code are supported for GAS, NASM, MASM/armasm, Go assembly, and IBM HLASM. |
...
**Status: Shipped (Go, Python, Java, JS/TS, Rust, Elixir, C).** The "What landed" notes below
