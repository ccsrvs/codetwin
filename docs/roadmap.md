...
| Assembly | **Not planned** | `--suggest` rejects assembly pairs with a note to factor the shared instructions into an assembler macro; there is no function abstraction to extract into. Detection, partial clones, and dead code are supported for GAS, NASM, MASM/armasm, and Go assembly. |
