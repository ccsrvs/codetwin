...
func synthesizeByLang(a, b scan.Snippet, pairID string, al Alignment) Suggestion {
	...
	case tokenizer.AsmGAS, tokenizer.AsmNASM, tokenizer.AsmMASM, tokenizer.AsmPlan9, tokenizer.AsmHLASM:
...
var (
	...
	cControlFlowKeywords      = []string{"return", "break", "continue", "goto"}
)
...
// synthesizeC produces a starter helper for two C function definitions
// under the same v1 contract as the other emitters: a literal copy of
// A's body, divergence comments, no parameterization. The header keeps
// A's storage class, attributes, return type, and parameter list —
// including GNU-style line breaks and K&R declarations — with every
// occurrence of the name in it replaced. Placement (above A's function,
// since C needs a declaration before use) is the patch layer's job.
//
// Definitions generated by a macro (TEST(suite, name) { … }) are
// rejected: the helper would have to be a macro too.
func synthesizeC(a, b scan.Snippet, pairID string, al Alignment) Suggestion {
	for _, s := range []scan.Snippet{a, b} {
		if strings.Contains(SymbolForSnippet(s), "@") {
			return Suggestion{Note: "rejected: snippet is a macro-generated definition (e.g. TEST(...)); refactor the macro instead"}
		}
	}
	return synthesizePair(a, b, pairID, al, pairEmitter{
		commentPrefix:    "//",
		cfKeywords:       cControlFlowKeywords,
		headerRejectNote: "rejected: snippet has no recognisable C function header",
		header:           cHelperHeader,
		rebody:           cRebodyAsHelper,
	})
}

// cHelperHeader returns A's definition header through the body's
// opening brace, with every occurrence of the function name — outside
// comments and literals — replaced by helperName.
func cHelperHeader(aCode, helperName string) (string, bool) {
	_, names, brace, ok := splitter.CDefinitionHeader(aCode)
	if !ok {
		return "", false
	}
	...
	last := 0
	for _, n := range names {
		b.WriteString(aCode[last:n[0]])
		b.WriteString(helperName)
		last = n[1]
	}
	b.WriteString(aCode[last : brace+1])
	return b.String(), true
}

// cRebodyAsHelper returns everything after the body's opening brace, in
// braceRebody's shape: the rest of the brace line (if any), then every
// following line through the closing brace.
func cRebodyAsHelper(aCode string) string {
	_, _, brace, ok := splitter.CDefinitionHeader(aCode)
	if !ok {
		return braceRebody(aCode)
	}
	rest := strings.TrimRight(aCode[brace+1:], "\n")
	first, following, _ := strings.Cut(rest, "\n")
	var body []string
