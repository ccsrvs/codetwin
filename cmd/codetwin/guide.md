...
`__tests__/`, `src/test/`, Rust `tests/`, `*_test.exs` / `test/`, C
`test_*` / `*_test` / `test-*` / `tst-*` / `tests/`) are classified as
test code by path. Test↔test pairs and clusters whose
...
- A C **prototype** or forward declaration names a function without
  ...
  the advisory tier because every includer can call it.
...
listing exactly what differs (`//` for Go/Java/JS-TS/Rust/C, `#` for
...
- **Language coverage in v1.** Go, Python, Java, JavaScript/TypeScript,
  Rust, Elixir, and C all emit helpers. For C the helper is inserted
  above the source function and its doc comment, because C needs a
  declaration before use; definitions generated by a macro
  (`TEST(suite, name) { … }`) are rejected. Assembly pairs are
  rejected with a note: factor the shared instructions into an
  assembler macro by hand.
